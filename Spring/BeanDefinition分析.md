## BeanDefinition的分析
首先BeanDefinition时什么？BeanDefinition时Spring的Ioc容器将bean的配置信息变成了一个对象，这个对象就是BeanDefinition。
BeanDefinition时一个接口，它规定了一个bean的配置信息中可以有什么东西。下面的是BeanDefinition的继承图:
![BeanDefinition的继承图](/spring/images/BeanDefinition的继承图.png)
下面的是AttributeAccessor的源码:
```
...
```
下面的是BeanDefinition的源码：
```java
public interface BeanDefinition extends AttributeAccessor, BeanMetadataElement {

	/**
	 * 标识bean 的scope为 singleton
	...
	/**
	 * 如果bean存在父类bean的话，设置父类bean的名称
	 * Set the name of the parent definition of this bean definition, if any.
	 */
	void setParentName(@Nullable String parentName);

	/**
	 * 返回此bean定义的父bean的名称，如果bean的定义有<bean parent=""的话
	...
	/**
	 * 设置懒加载 <bean lazy-init="true/false" >
	 ...
	 * @see #ROLE_APPLICATION	用户bean
	 * @see #ROLE_SUPPORT		复杂配置的bean
	...
	/**
	 * 返回该bean的定义来自的资源描述（用于在出现错误是显示上下文）
...
```
### AbstractBeanDefinition的分析
AbstractBeanDefinition虽然是抽象类，但是它完成了BeanDefinition的大部分的实现。
下面的是它的继承图：
![AbstractBeanDefinition的继承图](/spring/images/AbstractBeanDefinition继承图.png)
下面的是它的源码分析：
```
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	@Nullable
	private volatile Object beanClass;	// bean对应的实现类

	@Nullable
	private String scope = SCOPE_DEFAULT;	// Bean的作用域

	private boolean abstractFlag = false;	// 是否延迟初始化

	private boolean lazyInit = false;		// 是否为抽象bean

	private int autowireMode = AUTOWIRE_NO;	// 自动装配的模式

	private int dependencyCheck = DEPENDENCY_CHECK_NONE;	// 依赖检查的模式

	@Nullable
	private String[] dependsOn;		 // bean所依赖的bean的名称列表

	private boolean autowireCandidate = true;	// 是否为自动装配的候选者

	private boolean primary = false;			// 是否为自动装配的主候选者
	...
	private int role = BeanDefinition.ROLE_APPLICATION;	// Bean的角色
